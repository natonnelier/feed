...
var Feed = function () {
  ...
  _createClass(Feed, [{
    ...
    value: function atom1() {
      ...
      var feed = [{ _attr: { xmlns: 'http://www.w3.org/2005/Atom' } }, { title: options.title }, { published: options.published }, { generator: options.generator || GENERATOR }];
      ...
      this.items.forEach(function (item) {
        ...
        var entry = [{ title: { _attr: { type: 'html' }, _cdata: item.title } }, { id: item.id || item.link }, { link: [{ _attr: { href: item.link } }] }, { published: item.published }];
        ...
        // created
        if (item.image) {
          entry.push({ created: item.created });
        }
        ...
        // images
        if (item.image) {
          entry.push({ image: item.image });
        }

        if (item.image_small) {
          entry.push({ image_small: item.image_small });
        }

        if (item.image_medium) {
          entry.push({ image_medium: item.image_medium });
        }
    ...
    value: function json1() {
      ...
      feed.items = items.map(function (item) {
        ...
        if (item.date) {
          feedItem.date_modified = item.date;
        }
        if (item.published) {
          feedItem.date_published = item.published;
        }
